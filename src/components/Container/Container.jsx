        ...
        <main className="container-main">
          {listShows.map(resultShow => (
            <div className="movie" key={resultShow.id}>
              ...
              <p>
                Original Title :
                <span> {resultShow.original_name || resultShow.original_title}</span>
              </p>
              <img
                src={
                  resultShow.poster_path === null
                    ? notfound
                    : 'https://image.tmdb.org/t/p/w200' + resultShow.poster_path
                }
                alt={resultShow.title || resultShow.name}
              />
              <p>
                Popularity : <span> {resultShow.popularity}</span>
              </p>
              <p>
                Vote Average :<span> {resultShow.vote_average}</span>
              </p>
              <p>
                Vote Count :<span> {resultShow.vote_count}</span>
              </p>
              {resultShow.release_date === undefined ? (
                <p>
                  First Air Date :<span>{resultShow.first_air_date}</span>
                </p>
              ) : (
                <p>
                  Release Date :<span>{resultShow.release_date}</span>
                </p>
              )}
              <p>Genres :</p>
